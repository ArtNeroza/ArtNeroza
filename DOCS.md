...
### The tagline typewriter

`index.astro` retypes the eyebrow line on a 10s cycle. The full string is in the
HTML, so crawlers and no-JS visitors get it intact — the script only takes over
once it runs, swapping in a `.sr-only` copy for screen readers and an
`aria-hidden` span it animates.

The script locks `min-height` on the element before typing. Without it the line
starts one line tall and grows to two on narrow screens, shifting everything
below and wrecking CLS. It re-measures on resize. `prefers-reduced-motion`
skips the animation entirely and leaves the static text.
